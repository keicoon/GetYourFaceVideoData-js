        ...
        function get_video_info(path_video) {
            return new Promise((resolve, reject) => {
                ffmpeg(path_video).ffprobe(function (err, metadata) {
                    if (err) return reject('fail get_video_info', name_video, ":", err);
                    // @INFO: safety
                    const { start_time, duration } = metadata.format;
                    if (Number.isInteger(start_time) && Number.isInteger(duration)) {
                        resolve(metadata.format)
                    } else {
                        reject('fail get_video_info start_time', start_time, 'duration', duration);
                    }
                });
            });
        }
