...
module.exports = async (path_subtitle, path_mp3, info, options) => {
    ...
    return new Promise(async (resolve) => {
        ...
        loud && console.log('End split_audio_by_silence');

        const paths_mp3 = await utils.generate_splited_audio(time_table, path_mp3, name_audio);
        loud && console.log('End generate_splited_audio');

        const words = await utils.get_sst_api(options.sst_api)(paths_mp3);
        loud && console.log('End sst_api_func', time_table.length, '==', words.length);
        ...
        loud && console.log('generate subtile path:', path_subtitle);
