...
module.exports = (url, _file_name, _path = __dirname, options) => {
    ...
    logger.info('[dl_subs] reqest', url, _file_name, _path);
    ...
    return new Promise((resolve, reject) => {
        youtubedl.getSubs(url, {
            ...
            logger.debug('subtitle files downloaded:', files);
            ...
            const txt = await fs.readFileSync(path_new, 'utf8')
            const json = await vttToJson(txt);
            await fs.writeFileSync(path_final, JSON.stringify(json, null, 4), 'utf8');

            const dont_save_vtt = true;
            if (dont_save_vtt) fs.unlinkSync(path_new);
