  ...
  return new Promise(resolve => {
    ...
    vttArray.forEach((line, index) => {
      if (line.replace(/<\/?[^>]+(>|$)/g, "") === " ") {
        ...
        current = {
          start: timeString2s(line.split("-->")[0].trimRight().split(" ").pop()),
          end: timeString2s(line.split("-->")[1].trimLeft().split(" ").shift()),
          part: ''
        }
      } else if (line.replace(/<\/?[^>]+(>|$)/g, "") === "") {
      } else if (line.replace(/<\/?[^>]+(>|$)/g, "") === " ") {
    ...
    sections.forEach(section => {
      ...
      let section_word = [], section_start = start, section_end = end;
      ...
      cleanArray.forEach(function (item, idx) {
        if (item.indexOf('==') > -1) {
          ...
          let time = timeString2s(item.split("==")[1]);
          // resultsArray.push({
          //   id: id++,
          ...
          //   end: time
          // })
          section_word.push(cleanWord(item.split("==")[0]))
          start = time;
          valid_text = true;
        } else {
          if (valid_text && LastIdxCleanArray == idx) {
            ...
            //   end
            // })
            section_word.push(cleanWord(item))
            resultsArray2.push({
              part: section_word.join(' '),
              start: section_start,
              end: section_end
            });
          }
    ...
    resolve(resultsArray2);
  })
}

function timeString2s(a, b) {
  return timeString2ms(a, b) * 0.001;
}
// helpers
